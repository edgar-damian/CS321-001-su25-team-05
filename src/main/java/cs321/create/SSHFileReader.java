        ...
        switch (treeType) {
            ...
            case "reverseaddress-ip":
                if (line.contains("reverse")) {
                    ...
                    //Matcher m = Pattern.compile("reverse\\s+((?:\\d{1,3}\\.){3}\\d{1,3})").matcher(line);
                    ...
                    Matcher m = Pattern.compile("Address\\s+(\\d{1,3}(?:\\.\\d{1,3}){3})").matcher(line); //looks for "Address [ip addy]"
            ...
            case "user-ip":
                if (line.contains("Accepted") || line.contains("Failed") || line.contains("Invalid")) {
                    ...
                    if (ipMatcher.find()) {
                        ...
                        // Everything before ip
                        String beforeIp = line.substring(0, ipMatcher.start());

                        Matcher userMatcher = Pattern.compile("([*\\.\\\\a-zA-Z0-9_-]{1,64})").matcher(beforeIp); //grab 1-64 bytes of words (can be w special chars ['.', '*', ect]
                        String user = null;
                        ...
                        if (user != null) {
                            ...
                            user = user.replaceAll("[\\\\.]+$", "").trim(); // deletes possible white space or junk

                            // Catch long usernames
                            if (user.length() > 32) {
                                //System.out.println("Before cut: " + user);
                                user = user.substring(0, 32);
                                //System.out.println("After cut: " + user);
                            }

                            //return user + "-" + ip;
                            String returnedString = user + "-" + ip;
                            if (returnedString.length() > 32) {
                                //System.out.println("Before cut: " + returnedString);
                                returnedString = returnedString.substring(0, 32);
                                //System.out.println("After cut: " + returnedString);
                            }
